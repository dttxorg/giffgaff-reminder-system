...
- 用户用账号(或手机号)登录。账号可以是手机号(老用户无感迁移)或自定义账号(如 alice_2024)
- 一个账号 = 一张 SIM 卡的保号提醒(1:1)。想多张 SIM?用多个卡密,产生多个独立账号
- 绑定 Sever酱 / Bark / pushplus / Telegram 4 种推送渠道
- 管理员可批量生成 / 批量导入卡密(后者适合分销商给批次)
...
- **多号码汇总提醒**:账号有 4 张及以上活跃 SIM 时，提醒频率由最接近保号日期的号码决定；同一 bucket 只推送 1 条账号汇总，正文仅列尾号与已激活天数，并引导登录 `/me` 统一处理
